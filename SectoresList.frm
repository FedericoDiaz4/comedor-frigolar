...
Begin VB.Form SectoresList 
   ...
   Begin VB.Frame frmBuscar 
      ...
      Begin VB.ComboBox cboBuscar 
         ...
         EndProperty
         Height          =   360
         ItemData        =   "SectoresList.frx":1628
         Left            =   120
         List            =   "SectoresList.frx":1632
   ...
   End
End
Attribute VB_Name = "SectoresList"
...
Private Sub cmdEliminar_Click()
    ...
    Select Case MsgBox("¿DESEA ELIMINAR EL SECTOR " & Flex.TextMatrix(Flex.Row, 1) & "?", vbYesNo Or vbQuestion Or vbDefaultButton2, App.Title)
        Case vbNo: Exit Sub
    End Select
    
    Set rsDelete = New ADODB.Recordset
    SQL = "UPDATE sectores SET eliminado = 1 WHERE id = " & Flex.TextMatrix(Flex.Row, 0)
...
Private Sub cmdModificar_Click()
    ...
    Sectores.Nuevo = False
    Sectores.id = Flex.TextMatrix(Flex.Row, 0)
    
    Set rsCliente = New ADODB.Recordset
    SQL = "SELECT * FROM sectores WHERE id = " & Sectores.id
    rsCliente.Open SQL, Data, adOpenKeyset, adLockOptimistic
    Sectores.txtNombre.Text = rsCliente!nombre
    rsCliente.Close
...
Sub Cargar()
    ...
    SQL = "SELECT id, nombre FROM sectores WHERE eliminado = 0"
