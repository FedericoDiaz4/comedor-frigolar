...
Begin VB.Form MenuList 
   Caption         =   "Listado de Menus"
   ClientHeight    =   5760
   ClientLeft      =   120
   ClientTop       =   405
   ClientWidth     =   9855
   ...
   Begin VB.Frame frmBuscar 
      ...
      Begin VB.ComboBox cboBuscar 
         ...
         EndProperty
         Height          =   360
         ItemData        =   "EmpresasList.frx":1628
         Left            =   120
         List            =   "EmpresasList.frx":1632
...
Private Sub cmdEliminar_Click()
    ...
    Select Case MsgBox("¿DESEA ELIMINAR EL MENU " & Flex.TextMatrix(Flex.Row, 1) & "?", vbYesNo Or vbQuestion Or vbDefaultButton2, App.Title)
        Case vbNo: Exit Sub
    End Select
    
    Set rsDelete = New ADODB.Recordset
    SQL = "UPDATE menus SET eliminado = 1 WHERE id = " & Flex.TextMatrix(Flex.Row, 0)
...
Private Sub cmdModificar_Click()
    ...
    MENUS.Nuevo = False
    MENUS.id = Flex.TextMatrix(Flex.Row, 0)
    
    Set rsCliente = New ADODB.Recordset
    SQL = "SELECT * FROM menus WHERE id = " & MENUS.id
    rsCliente.Open SQL, Data, adOpenKeyset, adLockOptimistic
    MENUS.txtNombre.Text = rsCliente!nombre
    MENUS.txtCodigo.Text = rsCliente!codigo
    MENUS.txtPrecio.Text = rsCliente!Precio
    rsCliente.Close
...
Sub ordenaFlex()
    ...
    Flex.FormatString = "id|Código|Nombre|Precio"
    Flex.ColWidth(0) = 0
    Flex.ColWidth(1) = 800
